...
class CSPPF(nn.Module):
    ...
    def forward(self, x):
        ...
        y2 = self.m(y1)
        y3 = self.m(y2)
        z = self.cv2(torch.cat((x,y1,y2,y3),1))
        out = x_res+z
        return out
...
class C2f_Sim(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        ...
        self.cv1 = Conv(c1, 2 * self.c, 1, 1)
        self.cv2 = Conv((2 + n) * self.c, c2, 1)  # optional act=FReLU(c2)
        self.m = nn.ModuleList(SimBlock(self.c, self.c, shortcut, g, k=((3, 3), (3, 3)), e=1.0) for _ in range(n))
...
class SimBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        return x + self.sim(self.cv2(self.cv1(x))) if self.add else self.sim(self.cv2(self.cv1(x)))
