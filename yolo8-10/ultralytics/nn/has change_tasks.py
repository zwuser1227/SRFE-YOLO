...
class BaseModel(nn.Module):
    ...
    def predict(self, x, profile=False, visualize=False, augment=False, embed=None):
        ...
        if self.sr:
            x,output_sr=self._predict_once(x, profile, visualize, embed)
            return x,output_sr
        else:
    ...
    def _predict_once(self, x, profile=False, visualize=False, embed=None):
        ...
        for m in self.model:
            ...
            x = m(x)  # run
            # ----------这里的m.i==8如果修改了模型的层数记得改-----------
            if self.sr:
                y.append(x if m.i in self.save or m.i==8 else None)  # save output
            else:
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolov8n.yaml", ch=3, nc=None, verbose=True,sr=False):  # model, input channels, number of classes
        ...
        if self.sr == True:
            ...
            self.model_up = DeepLab(3,self.yaml['c1'],self.yaml['c2'],factor=2)#.cuda() #'if the size is m:192,768 l:256,1024 x:320 1280
